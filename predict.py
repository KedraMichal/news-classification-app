...
def modify_app_database(file_name, model, input_words, output_categories, update=False):
    ...
    for article in data:
        ...
        if np.max(model.predict(text)) > 0.7:
    ...
    with open(file_name, 'w', errors='ignore') as outfile:
        json.dump(database, outfile, ensure_ascii=False, indent=2, default=str)

    clear_json(file_name)


stemmer = StempelStemmer.default()
main_model = keras.models.load_model("resource/generated/model.h5")
with open('resource/generated/input_layer_words.txt', 'r') as words:
    input_layer_words = json.load(words)
output_layer_categories = ['Sport', 'Zdrowie', 'Biznes', 'Rozrywka', 'Technologie']

#modify_app_database("resource/rmm.json", main_model, input_layer_words, output_layer_categories, True)
